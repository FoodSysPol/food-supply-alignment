...
#Step 3: Obtain list of food items and item codes (see spreadsheet in teams)
```{r}
...
#Step 4: Identify countries missing in earlier data years and substitute values
```{r}
...
#Step 5: Correct for waste, loss, etc.
```{r}
...
#Step 6: Collapse food supply values over four food categories
```{r}
fao$group <- 0
...
table(is.na(guide$fruitveg.guideline), guide$WHO.region)
table(is.na(guide$sugar.guideline), guide$WHO.region)
table(is.na(guide$meat.guideline), guide$WHO.region)
table(is.na(guide$fish.guideline), guide$WHO.region)
...
regionIQR$quarter <- round(regionIQR$quarter)
regionIQR$threequarter <- round(regionIQR$threequarter)
write.table(regionIQR, file = "WHOregionIQR.txt", sep = ",", quote = FALSE, row.names = F)
...
#Step 15a: Create weighted mean (by region)
```{r}
#Create weighted mean of ratio (by income)
fruitvg <- foodgroup %>%                                  # Weighted mean by group
...
#Step 15b: Create weighted mean (by WHO region)
```{r}
#Create weighted mean of ratio (by WHO region)
fruitvg <- foodgroup %>%                                  # Weighted mean by group
  group_by(Year.x,WHO.region) %>% 
  summarise(weighted.mean(fruitveg.ratio, Value.pop, na.rm=TRUE))
...
names(wtreg)[names(wtreg) == 'weighted.mean.meat.ratio..Value.pop..na.rm...TRUE.'] <-'Red and processed meat'
names(wtreg)[names(wtreg) == 'weighted.mean.fruitveg.ratio..Value.pop..na.rm...TRUE.'] <-'Fruit and vegetables'
names(wtreg)[names(wtreg) == 'WHO.region'] <-'Region'
rm(meat, sugar, fish, fruitvg)
...
names(wtglobreg)[names(wtglobreg) == 'weighted.mean.meat.glob.ratio..Value.pop..na.rm...TRUE.'] <-'Red and processed meat'
names(wtglobreg)[names(wtglobreg) == 'weighted.mean.fruitveg.glob.ratio..Value.pop..na.rm...TRUE.'] <-'Fruit and vegetables'
names(wtglobreg)[names(wtglobreg) == 'WHO.region'] <-'Region'
rm(meat, sugar, fruitvg)
...
p <- ggplot(wt3, aes(x=Year, y=Ratio, group=Region, color=Region))+
  ...
  geom_hline(yintercept=1)+
  labs(y = "Alignment (supply/guideline)", color="Region")+
  scale_color_manual(values=c("#FF33FF", "#66FF00", "#33CCFF", "#000000", "#FF9933", "#FF0000"))+
  #scale_y_continuous(limits = c(0,5.1))
  theme(legend.position = c(0.85,0.25))
...
#Step 24: WHO European region
```{r}
#Create data frames
europe <- foodgroup[foodgroup$WHO.region=="Europe",c(2,3,13,16,20,21,22,23,24,25,26)]

names(europe)[names(europe) == 'Year.x'] <-'Year'
names(europe)[names(europe) == 'Country.x'] <-'Country'
names(europe)[names(europe) == 'Income..World.Bank.'] <-'Income'

who <- europe[,c(1,2,5,6,7,8)]
whoglob <- europe[,c(1,2,9,10,11)]

names(whoglob)[names(whoglob) == 'sugar.glob.ratio'] <-'Sugar'
names(whoglob)[names(whoglob) == 'fruitveg.glob.ratio'] <-'Fruitvg'
names(whoglob)[names(whoglob) == 'meat.glob.ratio'] <-'Meat'

whoglob2 <- whoglob %>%
  pivot_longer(cols= -c(1,2), names_to = "Food", values_to = "Ratio")

whoglob2$Food <- as.factor((whoglob2$Food))

names(who)[names(who) == 'sugar.ratio'] <-'Sugar'
names(who)[names(who) == 'fruitveg.ratio'] <-'Fruitvg'
names(who)[names(who) == 'meat.ratio'] <-'Meat'
names(who)[names(who) == 'fish.ratio'] <-'Fish'

who2 <- who %>%
  pivot_longer(cols= -c(1,2), names_to = "Food", values_to = "Ratio")

who2$Food <- as.factor((who2$Food))
...
#Meat compared to national FBDGs
meat <- who2[who2$Food=="Meat",]
meat <- subset(meat, (Country == "Denmark" | Country =="Estonia" | Country == "France" | Country == "Greece" | Country == "Ireland" | Country == "Italy" | Country == "Malta"))
p <- ggplot(meat, aes(x=Year, y=Ratio, group=Country, color=Country))+
  geom_line(size=1)+
  #facet_wrap(~Food, ncol=4)+
  geom_hline(yintercept=1)+
  labs(y = "Alignment (supply/guideline)", color="Country")+
  scale_color_brewer(palette = "Paired")
  #scale_y_continuous(limits = c(0,5.1))
  #theme(legend.position = c(0.85,0.25))

#Meat compared to global FBDGs
meat <- whoglob2[whoglob2$Food=="Meat",]
meat <- subset(meat, (Country == "Denmark" | Country =="Estonia" | Country == "France" | Country == "Greece" | Country == "Ireland" | Country == "Italy" | Country == "Malta"))
p <- ggplot(meat, aes(x=Year, y=Ratio, group=Country, color=Country))+
  geom_line(size=1)+
  #facet_wrap(~Food, ncol=4)+
...
#Step 25: Africa focus
```{r}
#Create data frame
africa <- foodgroup[foodgroup$WB.region=="Sub-Saharan Africa",c(2,3,20:26)]

names(africa)[names(africa) == 'Year.x'] <-'Year'
names(africa)[names(africa) == 'Country.x'] <-'Country'

national <- africa[,c(1:6)]
global <- africa[,c(1,2,7:9)]

names(global)[names(global) == 'sugar.glob.ratio'] <-'Sugar'
names(global)[names(global) == 'fruitveg.glob.ratio'] <-'Fruitvg'
names(global)[names(global) == 'meat.glob.ratio'] <-'Meat'


global <- global %>%
  pivot_longer(cols= -c(1,2), names_to = "Food", values_to = "Ratio")

global$Food <- as.factor((global$Food))

names(national)[names(national) == 'sugar.ratio'] <-'Sugar'
names(national)[names(national) == 'fruitveg.ratio'] <-'Fruitvg'
names(national)[names(national) == 'meat.ratio'] <-'Meat'
names(national)[names(national) == 'fish.ratio'] <-'Fish'

national <- national %>%
  pivot_longer(cols= -c(1,2), names_to = "Food", values_to = "Ratio")

national$Food <- as.factor((national$Food))

#Draw figure (national)
national <- national[!is.na(national$Ratio),]
p <- ggplot(national, aes(x=Year, y=Ratio, group=Country, color=Country))+
  geom_line()+
  facet_wrap(~Food, ncol=4)+
  geom_hline(yintercept=1)+
  labs(y = "Alignment (supply/guideline)", color="Country")
...
#Draw figure (national)
p <- ggplot(global, aes(x=Year, y=Ratio, group=Country, color=Country))+
  geom_line()+
  facet_wrap(~Food)+
  geom_hline(yintercept=1)+
  labs(y = "Alignment (supply/guideline)", color="Country")
